...
def plot_spectrum(eigval_df, n_eigs_display, title, outpath):
    ...
    plt.subplot(gs[0])
    plt.stem(
        eigval_df["eig"][: n_eigs_display + 1], eigval_df["val"][: n_eigs_display + 1]
    )
    plt.subplot(gs[1])
...
def plot_heatmap(
    ...
    X = numutils.coarsen(np.nanmean, E, {1: coarse_factor}, trim_excess=True)
    ax.matshow(
        X,
        rasterized=True,
        extent=extent,
    ...
    for block_name in blocks.keys():
        for i, track_name in enumerate(blocks[block_name], level):
            ...
            kwargs.setdefault("cmap", "RdBu_r" if track_type == "divergent" else "Reds")
            ...
            X = numutils.coarsen(
                np.nanmean, np.array([x]), {1: coarse_factor}, trim_excess=True
            )
            im = ax.matshow(X, rasterized=True, extent=extent, origin="lower", **kwargs)
            ax.set_aspect("auto")
            ax.xaxis.set_visible(False)
...
def plot_scatters(eigs, bins, trackconfs, panels):
    ...
    for panel_name, track_list in panels.items():
        ...
        da = dsshow(eigs, norm="linear", cmap="viridis", ax=ax, **ds_options)
        ...
        for i, track_name in enumerate(track_list, 1):
            ...
            if track_type == "category":
                df = eigs.assign(z=bins[track_name].astype("category"))
                kwargs["ax"] = ax
                if "facecolor" in kwargs:
                    ax.set_facecolor(kwargs.pop("facecolor"))
                da = dsshow(df, aggregator=ds.count_cat("z"), **kwargs, **ds_options)
                ...
                kwargs.setdefault(
                    "cmap", "RdBu_r" if track_type == "divergent" else "Oranges"
                )
                kwargs.setdefault("vmin", 0)
                if "vmax" not in kwargs:
                ...
                if "facecolor" in kwargs:
                    ax.set_facecolor(kwargs.pop("facecolor"))
                da = dsshow(df, aggregator=ds.mean("z"), **kwargs, **ds_options)
                ax.set_title(track_name)
                ax.axvline(0, c="k", lw=0.5, ls="--", alpha=0.5)
                ax.axhline(0, c="k", lw=0.5, ls="--", alpha=0.5)
                plt.colorbar(da, cax=cax)
